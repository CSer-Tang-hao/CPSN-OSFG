...
def main(args):
    ...
    model = Model(scale_cls=args.scale_cls, num_classes=args.num_classes, backbone=args.model)
    if use_gpu:
        model = model.cuda()
    ...
    for epoch in range(args.max_epoch):
        ...
        train(epoch, model, criterion1, criterion2, optimizer, trainloader, learning_rate, use_gpu)
...
def train(epoch, model, criterion1, criterion2, optimizer, trainloader, learning_rate, use_gpu):
    losses = AverageMeter()
    ...
    for batch_idx, (images_train, labels_train, images_test, labels_test, pids) in enumerate(tqdm(trainloader)):
        ...
        ytest, out1, out2 = model(images_train, images_test, labels_train_1hot, labels_test_1hot)

        loss1 = criterion1(ytest, pids.view(-1))#4.4041

        loss2 = criterion2(out1, labels_test.view(-1))#1.5999
        loss3 = criterion2(out2, labels_test.view(-1))  # 1.5999

        loss = 0.5 * loss2 + 0.5 * loss3 + 0.1 * loss1 #5.2040
...
def test(model, testloader, use_gpu):
    ...
    with torch.no_grad():
        for batch_idx , (images_train, labels_train, images_test, labels_test) in enumerate(tqdm(testloader)):
            ...
            cls_scores = cls_scores.view(q_batch_size * num_test_examples, -1)
            labels_test = labels_test.view(q_batch_size * num_test_examples)
            #print(cls_scores.shape)#torch.Size([225, 5])
