...
from .FE import SpatialAttn, ChannelAttn
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, kernel=3):
        ...
        # self.rga_att4 = RGA_Module(512, 6 * 6, use_spatial=spa_on, use_channel=cha_on,
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.layer1(x)
        x = self.layer2(x)#torch.Size([1, 128, 21, 21])
        x = self.layer3(x)#torch.Size([1, 256, 11, 11])
        x = self.layer4(x)#torch.Size([140, 512, 6, 6])
...
# model=resnet12()
# # rga= RGA_Module(512, 6*6, use_spatial=True, use_channel=True,
